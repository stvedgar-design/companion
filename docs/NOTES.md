...
- `docs/CONTRACTS.md`: tipos y firmas de los módulos (parcialmente superado: en
  ...
  tokens de diseño. **`docs/PRINCIPIOS-DE-INGENIERIA.md` (DOC-005): lectura obligatoria antes de tocar la interfaz, el chat o los datos.**
...
- `Message` (store `chatMsgs`, un array por `chatId`): `role` (`user|char`),
  ...
  `meta?` (UI-001, solo `char`: `{ttftMs, totalMs, chars}` de esa respuesta; ausente = sin dato).
...
de chat por personaje; CI con gate de tests; versión visible en Ajustes; resumen de continuidad por chat (MEM-007, **apagado por defecto**: cuesta ~+2-4 s por respuesta); 452
...
| UI-015 | Cursiva y color de Penumbra; una tipografía distinta por skin | Autorizado — **cerrado el 2026-09-26**: la parte del bug (cursiva/color) **no se reproduce**, sin cambio de CSS; **tipografía por skin implementada**; verificado en el navegador (375×812); 439 tests; **pendiente de probar en el teléfono** | Glass→Figtree, Penumbra→Lora, Penumbra Claude→Source Serif 4 (variables, OFL, +~361 KB); Nomi sigue con Literata. Test nuevo contra fugas de `--color-em`. Ver "UI-015" (en `HISTORIAL.md`) y `DESIGN.md`. |
| UI-001 | Rendimiento del chat: render incremental, streaming con cadencia, `content-visibility`, prueba de fluidez y tiempos por respuesta | Autorizado — **implementado el 2026-09-26** (sesión G1); medido en el navegador con 200 mensajes (ver abajo); 452 tests; **pendiente de medir en el teléfono real (prueba de fluidez y "Última respuesta")** | `perf.js` (puro), `ui/fluency.js`, `Message.meta`. `renderMessages()` 12–17 ms → 1,7–2,1 ms al abrir; por respuesta, 3 reconstrucciones completas → 0 y ~63 búsquedas + ~688 asignaciones de HTML → 14. Ver "UI-001" (en `HISTORIAL.md`). |
